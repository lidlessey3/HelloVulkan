...
void HelloTriangleApplication::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties,
                                           VkImage &image, VkDeviceMemory &imageMemory, uint32_t arrayLayers, VkImageCreateFlags flags, uint32_t mipLevels) {
...
void HelloTriangleApplication::createTextureSampler() {
    ...
    samplerInfo.maxLod     = static_cast<float>(mipLevels); // how many mip map level (minum level of detail)
...
void HelloTriangleApplication::createTextureImage() {
    ...
    mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(texWidth, texHeight)))) /*how many times the biggest dimension can be divided by two*/ + 1 /*the base image*/;
    ...
    createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage,
                textureImageMemory, 1, 0, mipLevels);
    transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
void HelloTriangleApplication::generateMipMaps(VkImage image, VkFormat imageFormat, uint32_t width, uint32_t height, uint32_t mipLevels) {
    // Check if image format supports linear blitting
    VkFormatProperties formatProperties;
    vkGetPhysicalDeviceFormatProperties(physicalDevice, imageFormat, &formatProperties);

    if (!(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT)) {    // if I don't support it I chrash
    ...
    for (int i = 1; i < mipLevels; i++) {
        std::cout << "Generating mip level " << i << std::endl;
        ...
        blit.dstOffsets[1]                 = { (int32_t) width > 1 ? (int32_t) width / 2 : 1, (int32_t) height > 1 ? (int32_t) height / 2 : 1, 1 };
        ...
        // now that I have copied the image I can transfer that miplevel to read only
